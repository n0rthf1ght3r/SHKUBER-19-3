# Домашнее задание к занятию «Сетевое взаимодействие в Kubernetes» - Крюков Егор
...
**Описание:**
Развернут Deployment с двумя контейнерами (nginx и multitool).
- Для доступа внутри кластера создан Service типа **ClusterIP**.
- Для доступа снаружи создан Service типа **NodePort**.

**Манифесты:**
* [Deployment (Nginx + Multitool)](./deployment-multi-container.yaml)
* [Service ClusterIP](./service-clusterip.yaml)
* [Service NodePort](./service-nodeport.yaml)

### Результаты проверки

**Скриншот 1. Состояние ресурсов и внешний доступ (NodePort)**
На скриншоте виден статус Pods, Services и успешный ответ на запрос `curl` с хост-машины (снаружи кластера):
![Скриншот](./1.png)

**Скриншот 2. Внутренний доступ (ClusterIP)**
На скриншоте виден запуск временного пода внутри кластера и успешный ответ от сервиса по внутреннему DNS-имени:
![Скриншот](./2.png)
...
**Описание:**
Развернуты два отдельных приложения (Frontend и Backend). Настроен Ingress-контроллер для маршрутизации трафика:
* Запросы на `/` перенаправляются на Frontend (Nginx).
* Запросы на `/api` перенаправляются на Backend (Multitool) с использованием `rewrite-target`.

**Манифесты:**
* [Приложения (Frontend/Backend)](./task2-apps.yaml)
* [Ingress конфигурация](./ingress.yaml)

### Результаты проверки

**Скриншот 3. Статус Ingress и проверка маршрутизации**
На скриншоте виден адрес Ingress и результаты запросов `curl` к `localhost` (возвращает Nginx) и `localhost/api` (возвращает Multitool):
![Скриншот](./3.png)
